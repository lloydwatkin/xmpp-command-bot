...
Commander.prototype.handle = function(command, callback) {
    ...
    var self = this
    var config = this.config[this.command]
    var execCommand = config.command
    if ('help' === this.command) {
        return this._handleHelpRequest(callback)
    }
    if (this.arguments) {
        execCommand += ' ' + this.arguments
    }
    debug('Running command: ' + execCommand)
    exec(execCommand, this.config.options, function(error, stdout, stderr) {
        if (!config || !config.reply) {
            return callback()
        }
...
Commander.prototype._handleHelpRequest = function(callback) {
    ...
    Object.keys(this.config).forEach(function(key) {
        var message = key + ':\n\t- ' + (this.config[key].summary || this.config[key].description)
