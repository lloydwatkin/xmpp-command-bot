...
Xmpp.prototype._joinRoom = function() {
    ...
    stanza.c('x', { xmlns: this.NS_MUC }).c('history', { maxchars: '0' })
...
Xmpp.prototype._handlePresence = function(stanza) {
    ...
    if (x) return this._handleMucUser(x, stanza.attrs.from)
...
Xmpp.prototype._handleMucUser = function(x, from) {
    var status = x.getChild('status')
    var item = x.getChild('item')
    if (item && this.config.xmpp.muc.roles) {
        var role = item.attrs.role.toLowerCase()
        debug(
            'Presence update from ' +
            from + ' ' + role
        )
        this._roomMembers[from] = item.attrs.role
        if ('none' === role)
            delete this._roomMembers[from]
        return
    }
    if (status && status.attrs.code &&
...
Xmpp.prototype._checkIsAllowed = function(from, stanza) {
    ...
    attrs.to = fromJid
    var denied = new ltx.Element('message', attrs)
...
Xmpp.prototype._sendPresence = function() {
    debug('Sending bot presence to the XMPP server')
